...
namespace Guess_Who_Harry_Potter
{
    class ServerData
    {
        public playBoard frm;//הגדרת אובייקט של הטופס שבתוכו יתנהל המשחק/תוכנה
        ...
        public byte[] writeBuffer;//מערך של בתים ששולח מידע לשרת

        public ServerData(playBoard frm)
        {
            this.frm = frm;
        }
        ...
        public void ReadSocket()
        {
            ...
            while (true)// הלולאה אינסופית
            {
                try
                {
                    readBuffer = new Byte[500];//מערך מטיפוס בייט של מידע שהשרת ישלח לנו ישמר בתוך האובייקט readBuffer
                    ...
                    //else
                    //{
                        playBoard.mess = System.Text.Encoding.UTF8.GetString(readBuffer);
                        playBoard.mess = playBoard.mess.Substring(0, ret);
                        frm.Invoke(playBoard.myDelegate);// כל מה שאני מוסיפה לאפס אני רוצה שהוא יוכל להשתלב יחד עם התהליך
                    //}
